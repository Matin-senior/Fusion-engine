// fusion-engine/modules/analyzer/dependencyGraphBuilder.ts
import path from 'path';
import fs from 'fs'; // Used for path resolution validation (e.g., check file existence)
import chalk from 'chalk';

// Import AnalyzedProject and ASTDependency from the same 'analyzer' module
 ...
 */
function inferFileType(fileAnalysis: ASTDependency): FileNode['type'] {
...
export function buildDependencyGraph(analysis: AnalyzedProject): DependencyGraph {
  ...
  for (const [sourcePath, fileAnalysis] of Object.entries(analysis.analyzedFiles)) {
    ...
    for (const imp of fileAnalysis.imports) {
      ...
      // Corrected the typo: 'analyulatedFiles' -> 'analyzedFiles'
    ...
    if (fileAnalysis.jsxElementsUsed && fileAnalysis.jsxElementsUsed.length > 0) {
      for (const usedComponent of fileAnalysis.jsxElementsUsed) {
        const resolvedComponentPath = componentExportsMap.get(usedComponent.name);
    ...
    if (fileAnalysis.typesUsedInAnnotations && fileAnalysis.typesUsedInAnnotations.length > 0) {
      for (const typeUsed of fileAnalysis.typesUsedInAnnotations) {
        const resolvedTypePath = typeExportsMap.get(typeUsed.name);
